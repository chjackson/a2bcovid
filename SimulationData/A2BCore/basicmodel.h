...
struct run_params {
	...
	int ward_format_old; //Flag to use a previous formatting style for the ward_file data.  Data format now obsolete; code preserved for historical value.
	...
	int symptom_uncertainty_calc; //Flag to use a distribution of times in the case of the time of symptoms being unknown.  The distribution is an offset gamma distribution specified by p.ucta, p.uctb and p.ucto.  Default is 0.  Under the default the mean time of the distribution is used, rather than a full distribution.
	int subset; //Flag to make subsets
	int delta; //Flag to use parameters appropriate to the Delta variant
};
...
struct ijlike {
	...
	int category; //Consistent=1, Borderline=2, Unlikely=3
