...
##'
##' @param pat_file (Required) Specify the name of a file containing the basic
...
##'   data collected from Cambridge University hospitals. (Relevant command
##'   --uct_mean).  4.  Infection type : Equal to 1 if the individual is a
##'   patient and a community case (i.e. who could not have been infected by
##'   others in the dataset but who could potentially transmit the virus to
##'   others).  This was defined as being positive for the virus 48 hours before
##'   admission to hospital with no healthcare contact in the previous 14 days
##'   prior to admission).  Equal to 2 if the individual is a patient and not a
##'   community case (i.e. who could potentially transmit and receive
##'   infection).  Equal to 3 if the individual is a healthcare worker.  Whether
##'   or not an individual is a healthcare worker is set by this parameter.  5.
##'   Sequence ID : A code used to link the individual to genome sequence
##'   information.  This should match the header of the sequence corresponding
##'   to the individual in the accompanying .fasta file (see --ali_file for
##'   this).  6.  Date of sample collection : Used in evolutionary calculations.
##'   Date format should be dd/mm/yyyy.  7.  Sample received date : Currently
##'   not used in the calculation.
##'
##' @param mov_file  Data describing when specific health care workers were on
...
##'   for each date.
##'
##' @param ali_file  Specify the name of a file in FASTA format containing
##'   genome sequences.  This file must contain all required sequences,
##'   specified by the sequence ID in the data of pat_file.
##'
##' @param ward_file  Specify the name of a file containing the location of
...
##'   columns as follows: 1.  Individual ID (same as for --pat_file)  2.
##'   Cluster ID e.g. the name of the ward being studied.  3.  Infection type
##'   e.g. 'patient' or 'HCW' for health care worker.  4.  Availability of data
##'   e.g. 'patient_moves_available' 5 onwards.  Data of the location of a
##'   patient, in sets of three columns.  These specify in turn: i)   The name
##'   of the location of the individual e.g. WARD_01. ii)  The start date of the
...
##'
##' @param data_type Specifes the kind of data which is being used to perform
